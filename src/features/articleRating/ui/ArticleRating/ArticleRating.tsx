...
const ArticleRating = ({ className, articleId }: ArticleRatingProps) => {
    ...
    const { data, isLoading } = useGetArticleRating({ articleId, userId: userAuth?.id ?? '' });
    const [articleRateMutation] = useArticleRate();

    const handleArticleRate = useCallback((startsCount: number, feedback?: string) => {
        try {
            articleRateMutation({
                userId: userAuth?.id ?? '',
                articleId,
    ...
    return (
        <RatingCard
            ...
            feedbackTitle={t('Оставьте совй отзыв о статье, это поможет улучшить качество')}
            hasFeedback
            rating={rating?.rate}
        />
    );
